...
class StereoDepth(object):
    ...
    def get_rectify_image(self, imgL, imgR):
        ...
        cv2.imwrite('docs/rectifiedL.png', rectifiedL)
    def task(self, frameL, frameR, waitKey=5):
        ...
        cv2.imwrite('docs/rectifiedL.png', rectifiedL)
        cv2.imwrite('docs/rectifiedR.png', rectifiedR)

        rect_path = os.path.join(os.path.abspath(os.path.join(args.left_file, "../..")),'rectfied')
        file_name = os.path.basename(args.left_file)
        if not os.path.exists(rect_path):
            os.makedirs(rect_path)
        imgL_path=os.path.join(rect_path,file_name)
        cv2.imwrite(imgL_path, rectifiedL)

        rect_path = os.path.join(os.path.abspath(os.path.join(args.right_file, "../..")), 'rectfied')
        file_name = os.path.basename(args.right_file)
        if not os.path.exists(rect_path):
            os.makedirs(rect_path)
        imgR_path = os.path.join(rect_path, file_name)
        cv2.imwrite(imgR_path, rectifiedR)
        ...
        # gray
        show_left = cv2.imread(r'F:\data\2023\1024\left\cube_20231024_151131\rectfied\show-Cwl_713.png')
        self.show_3dcloud_for_pcl(show_left, rectifiedR, points_3d)
        # color
        # self.show_3dcloud_for_pcl(rectifiedL, rectifiedR, points_3d)
        # show_left=cv2.imread(r'F:\data\2023\1024\left\cube_20231024_151131\rectfied\show-Cwl_713.png')
...
def get_parser():
    ...
    left_file = "data/left/seg/hsv/L.png"
    right_file = "data/left/seg/hsv/R.png"
