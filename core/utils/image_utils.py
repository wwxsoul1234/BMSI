...
import core.drawSpectral as drawSpectral
import matplotlib
...
def addMouseCallback(winname, param, path, left_map1, left_map2, callbackFunc=None, drawS=None, info="%"):
    """
     添加点击事件
    :param winname:
    :param param:
    ...
    def default_callbackFunc(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            # print("image(x,y)=({},{}),".format(y, x) + info.format(param[y][x]))    #之前
            print("image(x,y)=({},{}),".format(x, y) + info.format(param[y][x]))

            # title = "depth = " + str(param[x][y][2])
            title = 'world coords = {}mm'.format(param[y][x])
            # if drawSpectral ==1:
            #     # wwx-add
            #     # read spectral
            #     ref = drawSpectral.L_TP_Reflect(path, x, y, left_map1, left_map2)
            #     drawSpectral.plot_Ref(ref, title)
            print(path)
            path1 = os.path.abspath(os.path.join(path, '../..'))
            spec_path = os.path.join(path1,'png')

            spec_path = r'F:\data\2023\1024\left\cube_20231024_151131\rectfied\no-seg'
            ref = drawSpectral.L_TP_Reflect(spec_path, x, y, left_map1, left_map2)
            # drawSpectral.plot_Ref(ref, title)
            # drawSpectral.plot_derivative(ref, title)
            drawSpectral.plt_REF_DER(ref, title)
